...
- [ ] **Pinterest catalog — submitted 2026-08-09, review takes 24–48h. Check back Mon Aug 11.**
      Feed: `https://www.dragoninkandthread.com/pinterest-catalog.csv` (33 products, generated by
      `node tools/build-catalog.js`). Nothing appears on Pinterest until it's approved.
      - **If approved:** the pieces become Product Pins. Nothing further to do.
      - **If rejected:** the feed itself was validated on 2026-08-09 — it parses, and every
        product page and photo it points at exists — so a rejection is far more likely to be
        *merchant* eligibility (a policy review of the account and site) than a broken file.
        Read the reason before changing anything; don't start editing the feed on a guess.
      - ⚠️ **From now on, when a piece sells: set `soldOut: true` first, retire it to Stories a
        few days later.** Retiring immediately deletes the product page, and a live Pin can
        then land on a 404 until Pinterest re-reads the feed.
